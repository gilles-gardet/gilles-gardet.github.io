...
/**
 * Retrieve a translation value by dot-separated key path.
 *
 * @param lang the active language
 * @param key the dot-separated key path (e.g. 'cv.contact.presentation')
 * @param params optional interpolation parameters (e.g. { value: 12 })
 * @returns the translated string, or the key if not found
 */
export function t(lang: Language, key: string, params?: Record<string, unknown>): string {
  const parts: string[] = key.split('.');
  let value: TranslationValue = translations[lang] as unknown as TranslationValue;
