    ...
    return actions$.pipe(
      ...
      switchMap(({ language }) =>
        languageService.setLanguage$(language).pipe(
          ...
          switchMap(() => [MissionActions.loadMissions(), SkillActions.loadSkills()]),
        ),
      ),
    );
  },
...
export const updateLoaderOnMissionLoad = createEffect(
  (actions$ = inject(Actions)) => {
    return actions$.pipe(
      ofType(
      ...
      bufferCount(2), // wait for both missions and skills to complete
